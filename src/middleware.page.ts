...
import { getMeForServer, isDashboardAccessible } from './utils/auth';
...
export async function middleware(request: NextRequest) {
  ...
  if (pathName.startsWith(PAGE_PATH.DASHBOARD)) {
    const dashboardId = pathName.split('/')[2];
    const isAccessible = await isDashboardAccessible(accessToken, dashboardId);

    if (!isAccessible) {
      return NextResponse.redirect(
        new URL(PAGE_PATH.MY_DASHBOARD, request.url),
      );
    }
  }
}
